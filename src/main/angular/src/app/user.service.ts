...
export class UserService {
  ...
  createUser(user) {
    return this.http.post("http://192.168.1.9/signup.php", JSON.stringify(user), {headers: this.headers})
  }

  // async loadPersonalInformation() {
  //   let email = localStorage.getItem('user')
  //   // 
  //   let res = await this.http.post("http://192.168.1.9/get_info.php", JSON.stringify(email), {headers: this.headers}).toPromise()
  ...
  //   
  //   let res = await this.http.post("http://192.168.1.9/get_app.php", JSON.stringify(cid), {headers: this.headers}).toPromise()
  ...
  async loadData() {
    let email = localStorage.getItem('user')
    await this.http.post("http://192.168.1.9/get_info.php", JSON.stringify(email), {headers: this.headers}).subscribe(res => {
      ...
      let cid = this._personalInformation.citizenID
       this.http.post("http://192.168.1.9/get_app.php", JSON.stringify(cid), {headers: this.headers}).subscribe(res=>{
  ...
  updatePersonalInformation() {
    ...
    this.http.post("http://192.168.1.9/update_info.php", JSON.stringify(this._personalInformation), {headers: this.headers})
    .subscribe()
  }
  ...
  updateApplication(application) {
    ...
    this.http.post("http://192.168.1.9/update_app.php", JSON.stringify(application), {headers: this.headers})
    .subscribe()
  }
